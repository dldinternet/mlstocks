    ...
    def test_klass_sup_sub(self):
        with mock.patch('builtins.issubclass', return_value=False):
            try:
                ...
                assert str(
                    ie) == 'Class Config_OandAAPI is not one of our supported Configs! '\
                           'You are welcome to send in the request for it!'
    ...
    def test_klass_sup_path(self):
        with mock.patch('os.path.exists', return_value=True):
            try:
                ...
                ies = str(ie)
                ies.replace(path, '')
                assert ies.replace(path, '') == '/mlstocks/config/fake.py exists but '\
                                                'importing it failed! Please check the syntax ...'
